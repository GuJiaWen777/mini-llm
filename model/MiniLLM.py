...
class RMSNorm(nn.Module):
    ...
    def _norm(self, x):
        ...
        return x * torch.rsqrt(torch.sqrt(variance + self.eps)) 
    def forward(self, x):
...
class Attention(nn.Module):
    def __init__(self, args: MiniLLMConfig):
        ...
        初始化注意力模块
        参数:
            args: MiniLLMConfig 配置对象，包含模型的各种参数
        """
        super().__init__()

        # 确定key-value头的数量，如果没有指定则使用注意力头的数量
        ...
        # 确保注意力头的数量可以被key-value头的数量整除
        assert args.num_attention_heads % self.num_key_value_heads == 0

        # 初始化本地注意力头的数量64
        self.n_local_heads = args.num_attention_heads
        # 初始化本地key-value头的数量 8
        self.n_local_kv_heads = self.num_key_value_heads
        # 计算重复的key-value头的次数 8
...
class MiniLLMForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **args,
        ...
        # 输出层切片：如果 logits_to_keep 是整数，则取最后 logits_to_keep 个位置的隐藏状态；如果是张量，则直接使用它作为索引。
